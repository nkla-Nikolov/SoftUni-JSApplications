...
function eventsTemplate(events) {
    return html`
    <section class="welcomePage">
        ...
        <div id="events">
            ...
            <div class="theaters-container">
                ...
                ${events.length > 0 ? events.map(x => singleEventTemplate(x)) : html`<h4 class="no-event">No Events Yet...</h4>`}
...
function singleEventTemplate(event) {
    return html`
...
export async function getAllEvents(){
    let response = await fetch('http://localhost:3030/data/theaters?sortBy=_createdOn%20desc&distinct=title');
...
export async function getEventById(id){
    let response = await fetch(`http://localhost:3030/data/theaters/${id}`);
...
export async function homePage(){
    ...
    render(eventsTemplate(events), document.querySelector('#content'));
}